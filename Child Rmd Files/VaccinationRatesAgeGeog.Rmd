...
|Routt Residents (All Ages) Received Booster Dose|`r ((PtIZ %>% filter(dosage_num==3) %>%  count())+(PtIZ %>% filter(vaccination_code=="COVID-19 Vector-NR (JSN)") %>% select(patient_id) %>% distinct() %>% left_join(PtIZ %>% select(patient_id, vaccination_code, dosage_num, vaccination_date), by = "patient_id") %>% filter(vaccination_code!="COVID-19 Vector-NR (JSN)") %>% count())) %>% pull()`|`r scales::percent((((PtIZ %>% filter(dosage_num==3) %>%  count())+(PtIZ %>% filter(vaccination_code=="COVID-19 Vector-NR (JSN)") %>% select(patient_id) %>% distinct() %>% left_join(PtIZ %>%            select(patient_id, vaccination_code, dosage_num, vaccination_date), by = "patient_id") %>% filter(vaccination_code!="COVID-19 Vector-NR (JSN)") %>% count()))/params$RouttPop) %>% pull())`|
