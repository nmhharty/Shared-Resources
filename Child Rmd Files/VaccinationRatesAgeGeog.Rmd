...
|-----|------|-----|
|Routt Residents (All Ages) Received at Least 1 Dose|`r PtIZ %>% filter(dosage_num==1) %>% count() %>% pull()`|`r scales::percent((PtIZ %>% filter(dosage_num==1) %>% count() %>% pull())/params$RouttPop)`|
|Routt Residents (All Ages) Completed Vaccine Series|`r PtIZ %>% filter(FullyVax=="Yes") %>% count() %>% pull()`|`r scales::percent((PtIZ %>% filter(FullyVax=="Yes") %>% count() %>% pull())/params$RouttPop)`|
...
|Routt Residents Ages 5-11 Received at Least 1 Dose|`r PtIZ %>% filter(dosage_num==1, age_at_1stvaccination>4, age_at_1stvaccination<12) %>%  count() %>% pull()`|`r scales::percent((PtIZ %>% filter(dosage_num==1, age_at_1stvaccination>4, age_at_1stvaccination<12) %>%  count() %>% pull())/(RouttPopTable2 %>% filter(AGE>4,AGE<12) %>% summarise(TotalPop=sum(TOTAL))) %>% pull())`|
|Routt Residents Ages 5-11 Completed Vaccine Series|`r PtIZ %>% filter(FullyVax=="Yes", age_at_1stvaccination>4, age_at_1stvaccination<12) %>% count() %>% pull()`|`r scales::percent((PtIZ %>% filter(FullyVax=="Yes", age_at_1stvaccination>4, age_at_1stvaccination<12) %>%  count() %>% pull())/(RouttPopTable2 %>% filter(AGE>4,AGE<12) %>% summarise(TotalPop=sum(TOTAL))) %>% pull())`|  
...
PtIZ %>%
  ...
  add_row(City = "Routt County Overall", 
          `Percent Received At Least One Dose` = scales::percent((PtIZ %>% filter(dosage_num==1) %>% count() %>% pull())/params$RouttPop),
