---
title: "INTERNAL USE ONLY: Routt County COVID-19 Dashboard"
author:
...
DataThroughDate <- as.Date.character("2021-10-25")
MostRecentWeekStart <- as.Date.character("2021-10-18")
...
|-----|-----|-----|-----|
|Low Risk|Fewer than 200 cases per 100,000 (<52 cases) in two weeks **OR** At least 75% of Routt residents aged 16+ complete the COVID-19 vaccination series |Less than 5% |Sufficient local hospital capacity|
...
**Total Tests Administered:** `r DeDupNonAntibodyTests %>% ungroup() %>% filter(CollectionDate<=(DataThroughDate)) %>% summarise(n())`  
...
|Routt County Resident COVID-19 Hospitalizations|`r COPHS %>% filter(IsMostRecentWeekRolling=="Yes") %>% summarise(Count = n_distinct(PersonID)) %>% pull(Count)`|`r COPHS %>% filter(IsMostRecent2WeeksRolling=="Yes") %>% summarise(Count = n_distinct(PersonID)) %>% pull(Count)`|
|COVID-19 Hospitalizations at Yampa Valley Medical Center|`r Hospit %>% filter(Hospital.Name=="Yampa Valley Medical Center"&IsMostRecentWeekRolling=="Yes") %>% summarise(Count = n()) %>% pull(Count)`|`r Hospit %>% filter(Hospital.Name=="Yampa Valley Medical Center"&IsMostRecent2WeeksRolling=="Yes") %>% summarise(Count = n()) %>% pull(Count)`|
...
```{r label=VaccinationsByAgeGroup}
PtIZ %>%
  filter(FullyVax=="Yes") %>%
...
```{r label=VaccinationsByAgeGroup10yr}
PtIZ %>%
  filter(FullyVax=="Yes") %>%
...
```
```{r label=DailyAvgCasesPerDay14day}
plot_ly(ConfProbCases %>%
          filter(#CountsTowardTotal=="Yes", 
                 AttributionDate<=(DataThroughDate)) %>%
          group_by(AttributionDate) %>%
...
add_trace(data = (ConfProbCases %>%
                    ...
                    arrange(AttributionDate) %>%
                    filter(AttributionDate<=(DataThroughDate)) %>%
                    mutate(Case14daDailyAvg = zoo::rollmeanr(NumberCases, k = 14, fill = NA))),
          x = ~AttributionDate,
          y = ~Case14daDailyAvg,
          type = "scatter",
          color = I("#4E2B1F"),
          mode = "lines",
          yaxis = "y2",
          name = "Rolling 14-day Daily average Cases by Specimen Collection Date") %>%
  layout(title = "Cases per Day with Rolling 14-day Average Daily Cases",
          margin = list(l=50,r=50,h=175),
         xaxis = list(type = 'date',
                      tickformat = "%b %d, %Y",
                      autotick = F, 
         ...
         yaxis2 = list(
           ...
           side = "right",
           showgrid = FALSE,
           title = "14-day Rolling Average Daily Cases",
           rangemode = "tozero",
           range = c(0,42)),
         legend = list(orientation = 'h', y=-.3),
         font = list(family = "Arial", size = 12),
...
ConfProbCases %>%
  ...
  filter(ReportedDate<=(DataThroughDate)) %>%
  mutate(Case14da = zoo::rollsumr(NumberCases, k = 14, fill = NA)) %>%
...
## 14-Day Average Number of Tests Administered and Average Positivity  
This graph shows the 7-day rolling average number of tests performed each day (bars) overlayed with the 7-day rolling average positivity rate (line). Clicking on the legend titles will toggle the bars or lines on and off. Since late May, Routt County has been completing well more than the minimum daily tests for our county of 38 per day.  
```{r label=Rollin7DayAvgTestsAdministeredPer100K}

plot_ly(data = (DeDupNonAntibodyTests %>%
                   group_by(CollectionDate) %>% 
                   summarise(TotalTests = n()) %>%
                  # replace(., is.na(.), 0) %>%
                   right_join((Calendar %>%
                                 select(date) %>%
                                 filter(date>"2020-02-29"&date<=(DataThroughDate))), by = c("CollectionDate" = "date")) %>%
                  replace(., is.na(.), 0) %>%
                   arrange(CollectionDate) %>%
                   mutate(Rolling14DayTestsAvg=round(zoo::rollmeanr(TotalTests, k = 14, fill = NA, na.rm=TRUE)),
                          Rolling14DayTestRate=round(((Rolling14DayTestsAvg/25652)*100000),2))), 
        x = ~CollectionDate,
        y = ~Rolling14DayTestsAvg,
        name = '14-day Rolling Average Daily Number of Tests performed',
        type = 'bar',
        color = I("#BB4628")) %>%
add_trace(data = (ConfProbCases %>%
                    filter(CountsTowardTotal=="Yes") %>%
...
ConfProbCases %>%
  ...
  filter(WeekStart<=(DataThroughDate)) %>%
...
```{r label=OutbreaksBarChartResolved}
library(googlesheets4)

Outbreaks %>%
  filter(Status!="Under Investigation", `Start Date`<=(DataThroughDate)) %>%
  mutate(Resolved = case_when(`End Date (Update as Needed)`<=(DataThroughDate) ~ "Resolved",
...
add_trace(y = ~ActivePct,
  ...
  layout(margin = list(t = 45),
         title = list(text = "Case Outcomes by Age",
                      yref = "container",
                      yanchor = "top",
                      pad = list(t = 15),
                      y = 1),
...
plot_ly(
  ...
  add_trace(y = ~NumberCasesWeek,
            ...
            yaxis = "y2") %>%
  layout(title = "Weekly Proportion of Symptomatic/Asymptomatic Cases (bars) <br>with Total Weekly Cases (line) CDPHE DATA",
         xaxis = list(type = 'date',
                      tickformat = "%b %d, %Y",
                      tickangle = 45,
                      autotick = F, 
...
Clinical trials for COVID-19 vaccines were designed to assess efficacy in preventing severe disease, hospitalization, and/or death. No vaccine is 100% effective. Therefore, we expect to see vaccine breakthrough cases, and we expect to see a small portion of cases (5-15%) among vaccinated individuals who do develop severe disease, hospitalization, or death. 
...
```{r label=VaxBreakthroughbyMonthGraph3}
PtIZ %>%
  filter(FullyVax=="Yes") %>%
 ...
 filter(FullyVaxMonthAttr!="Jan") %>%
  ...
  left_join(VaxBreakthroughList %>%
            mutate(AttributionMonth = month(AttributionDate, label = TRUE)) %>%
             filter(AttributionMonth!="Jan") %>%
              group_by(AttributionMonth) %>%
              summarise(VaxBreakthrough = n()) %>%
              left_join(ConfProbCases %>%
                          mutate(AttributionMonth = month(AttributionDate, label = TRUE)) %>%
                         filter(Age>11, AttributionMonth!="Jan") %>%
                          group_by(AttributionMonth) %>%
                          summarise(Total = n()), by = "AttributionMonth") %>%
...
```
The chart above would be more meaningful in biweekly or monthly aggregation. This is based upon code from CDPHE and I will need to do some re-working to change to a less granular aggregation.

```{r label=VaxBreakthroughDemos}
VaxBreakthroughList %>%
...
## Test Type  
On 12/1/2020, CDPHE anecdotally quoted that 5% of cases were determined by antigen test. The following tables give estimates for the proportion of Routt County cases with a test result that are determined by antigen versus PCR (first table) and the proportion of tests performed that are antigen versus PCR (second table).  
```{r label=AntigenPCRcases}
ConfProbCases %>%
  filter(!is.na(Spec1_CollectionDate)) %>%
  group_by(CaseStatus) %>%
  count() %>%
  mutate(TotalCasesWithTests = (ConfProbCases %>%
                                  filter(!is.na(Spec1_CollectionDate)) %>%
                                  count() %>%
                                  pull()),
         PercentageCases = scales::percent(n/TotalCasesWithTests)) %>%
  kable(caption = "Proportion of Cases Determined by Antigen (Probable) vs PCR (Confirmed)")

```

```{r label=AntigenPCRtests}
NonAntibodyTests %>%
  group_by(TestTypeGroup) %>%
  count() %>%
  mutate(TotalTests = (NonAntibodyTests %>%
                         count() %>%
                         pull()),
         PercentageCases = scales::percent(n/TotalTests)) %>%
  kable(caption = "Proportion of Antigen vs PCR Tests")
...
ConfProbCases %>%
  filter(!is.na(Spec1_CollectionDate)) %>%
  group_by(AttributionWeekStart, Spec1_Test1_Name) %>%
  summarise(Count = n()) %>%
  filter(Spec1_Test1_Name %in% c("RT-PCR", "Antigen for COVID-19")) %>%
  pivot_wider(names_from = Spec1_Test1_Name, values_from = Count) %>%
  mutate(TotalCases = sum(`RT-PCR`,`Antigen for COVID-19`, na.rm=TRUE),
         ProportionPCR = `RT-PCR`/TotalCases,
         ProportionAntigen = `Antigen for COVID-19`/TotalCases) %>%
  right_join((Calendar %>%
                select(WeekStart) %>%
                distinct()), by = c("AttributionWeekStart" = "WeekStart")) %>%
  replace(., is.na(.), 0) %>%
...
plot_ly(
  ...
  add_trace(y = ~TotalCases,
            ...
            name = 'Total Weekly Case Count',
            yaxis = "y2") %>%
  layout(title = "Weekly Proportion of Antigen and PCR Cases (bars)",
         xaxis = list(type = 'date',
                      tickformat = "%b %d, %Y",
                      tickangle = 45,
                      autotick = F, 
...
### Temporal Trends in Exposure Type  
...
ConfProbCases %>%
  filter(AgeGrouping %in% c("0-9", "10-19"), AttributionDate<=(DataThroughDate)) %>%
  group_by(AgeGrouping) %>%
  summarise(NumberCases = n()) %>%
  arrange(AgeGrouping) %>%
...
ConfProbCases %>%
  filter(Age<18, AttributionDate<=(DataThroughDate)) %>%
...
ConfProbCases %>%
  ...
  summarise(cases = n()) %>%
  mutate(AgeGroup = "Under20", Pop = 5798, Incidence = round((cases/Pop*1000),2)) %>%
  select(-cases) %>%
  rbind(ConfProbCases %>%
              summarise(cases = n()) %>%
              mutate(AgeGroup = "All", Pop = RouttPop, Incidence = round((cases/Pop*1000),2)) %>%
              select(-cases)) %>%
...
```{r label=tests0to17}
NonAntibodyTests %>%
  filter(AgeAtCollection<18, CollectionDate<=(DataThroughDate)) %>%
...
## Race  
```{r label=RaceCasesPieChartAll}
plot_ly() %>%
  add_pie(data = ConfProbCases %>%
            filter(AttributionDate<=(DataThroughDate)) %>%
            group_by(Ethnicity) %>%
            count(),
          labels = ~Ethnicity, values = ~n,
          marker = list(colors = c('#4E2B1F', '#BB4628', "#477691")),
          name = "Cases",
          title = "Cases",
          domain = list(row = 0, column = 0)) %>%
  ...
  #         domain = list(row = 0, column = 1)) %>%
  layout(margin = list(t=25,b=15),
         title = list(text = "Ethnicity Breakdown of COVID-19 Cases",
                      font = list(family = "Arial", size = 14)),
         grid=list(rows=1, columns=2))
```
...
**July 16, 2021**: Added Vaccine Breakthrough section with data on cases among fully vaccinated residents.  
Added 14-day incidence rate by geography.  
...
**School-age**: anyone 5 through 17 years old, inclusive, at the time of case report.  
