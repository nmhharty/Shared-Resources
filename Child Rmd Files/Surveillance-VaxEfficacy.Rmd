...
PtIZ %>%
  filter(dosage_num==3, age_at_1stvaccination>=18) %>%
  select(patient_id) %>%
  distinct() %>%
  count() %>%
  mutate(JOIN = "") %>%
  ...
  select(-JOIN) %>%
  mutate(PercentPop = scales::percent(n/POPTotal)) %>%
  kable(caption = "3rd Dose mRNA Recipients 18+")

PtIZ %>%
  filter(dosage_num==3, age_at_1stvaccination>=65) %>%
  select(patient_id) %>%
  distinct() %>%
  count() %>%
  mutate(JOIN = "") %>%
  ...
  select(-JOIN) %>%
  mutate(PercentPop = scales::percent(n/POPTotal)) %>%
  kable(caption = "3rd Dose mRNA Recipients 65+")
