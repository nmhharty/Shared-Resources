...
VaxCases <- Cases %>%
  select(eventid, first_name, last_name, date_of_birth, reporteddate, collectiondate, onsetdate_proxy_dist) %>%
...
VaxCases <- VaxCases %>%
  ...
  mutate(DaysVaxMinusTest = as.numeric(difftime(attribution_date,vaccination_date, units = "days")),
         breakthrough = case_when(DaysVaxMinusTest>14 ~ 1, 
                                  TRUE ~ 0)) %>%
...
##Below is for grouping cases by week
...
#Adding column to cases2 for rolling 14 day and 28 day cases
cases2 %>%
  right_join(Calendar %>%
               select(date), by = c("reporteddate" = "date")) %>%
